...
from models.PANNS_CRNN_mtl import *
...
def load_merged_data(_feat_folder, _lab_folder,  _fold=None):
    ...
    _Y_train, _Y_val, _Y_train_c, _Y_val_c, _Y_train_i, _Y_val_i = dmp['arr_0'], dmp['arr_1'], dmp['arr_2'], dmp['arr_3'], dmp['arr_4'], dmp['arr_5']

    return _X_train, _Y_train, _X_val, _Y_val, _Y_train_c, _Y_val_c, _Y_train_i, _Y_val_i


def preprocess_data(_X, _Y, _X_val, _Y_val, _Y_c, _Y_val_c, _Y_i, _Y_val_i, _seq_len):
    # split into sequences
    _X = split_in_seqs(_X, _seq_len)
    _Y = split_in_seqs(_Y, _seq_len)
    _Y_c = split_in_seqs(_Y_c, _seq_len)
    _Y_i = split_in_seqs(_Y_i, _seq_len)

    _X_val = split_in_seqs(_X_val, _seq_len)
    _Y_val = split_in_seqs(_Y_val, _seq_len)
    _Y_val_c = split_in_seqs(_Y_val_c, _seq_len)
    _Y_val_i = split_in_seqs(_Y_val_i, _seq_len)

    return _X, _Y, _X_val, _Y_val, _Y_c, _Y_val_c, _Y_i, _Y_val_i
...
def train():
    ...
    if(loss=='clip_mse'):
        loss_function = clip_mse
    elif(loss=='focal_loss'):
        loss_function = soft_label_focal_loss
    elif(loss=='kl_loss'):
        loss_function = kl_loss
    elif(loss=='curriculum_loss'):
        loss_function = curriculum_loss
    else:
        loss_function = soft_label_kl_loss
    # CRNN model definition   
    cnn_filters = 128       # Number of filters in the CNN
    rnn_hid = 256           # Number of RNN nodes.  Length of rnn_nb =  number of RNN layers
    ...
    for fold in holdout_fold:
        ...
        X, Y, X_val, Y_val, Y_c, Y_val_c, Y_i, Y_val_i = load_merged_data('development/features', 'development/soft_labels', fold)
        X, Y, X_val, Y_val, Y_c, Y_val_c, Y_i, Y_val_i = preprocess_data(X, Y, X_val, Y_val, Y_c, Y_val_c, Y_i, Y_val_i, seq_len)
        
        train_dataset = maestroDataset(X, Y, Y_c, Y_i)
        validate_dataset = maestroDataset(X_val, Y_val, Y_val_c, Y_val_i)
        ...
        pytorch_total_params = sum(p.numel() for p in modelcrnn.parameters())
        print('\nNumber of parameters:{}'.format(pytorch_total_params))
        
        pretrained_checkpoint_path = '/notebooks/ntu/dcase2023_task4b_baseline/Cnn14_mAP=0.431.pth'
        modelcrnn.load_from_pretrain(pretrained_checkpoint_path)
        ...
        for epoch in range(stop_iteration):
            ...
            modelcrnn.train()
            # TRAIN
            for (batch_data, batch_target, batch_target_category, batch_target_place) in train_loader:
                # Zero gradients for every batch
                optimizer.zero_grad()

                batch_output, batch_output_category, batch_output_place= modelcrnn(move_data_to_device(batch_data, device))

                # Calculate loss
                loss_o = clip_mse(batch_output, move_data_to_device(batch_target,device))
                loss_c = clip_mse(batch_output_category, move_data_to_device(batch_target_category,device))
                loss_i = clip_mse(batch_output_place, move_data_to_device(batch_target_place,device))
                
                loss = alpha*(loss_o) + (beta)*(loss_c) + (gamma)*(loss_i)      
            ...
            with torch.no_grad():
                ...
                running_loss = 0.0
                for (batch_data, batch_target, _, _) in validate_loader:
                    batch_output, _, _ = modelcrnn(move_data_to_device(batch_data, device))
        ...
        with torch.no_grad():
            ...
            for file in test_files:
                ...
                # Feed into the model
                batch_output, _, _ = modelcrnn(data[None,:,:].to(device))
